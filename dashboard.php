...
$expenseTotal = $conn->query("
SELECT SUM(amount) t FROM expenses 
WHERE user_id=$uid
")->fetch_assoc()['t'] ?? 0;

$incomeTotal = $conn->query("SELECT SUM(amount) t FROM transactions WHERE user_id=$uid AND type='income'")->fetch_assoc()['t'] ?? 0;
...
/* CATEGORY BUDGET STATUS QUERY */

$categoryBudgetStatus = $conn->query("
SELECT 
    c.category_name,
    IFNULL(SUM(e.amount),0) AS spent,
    IFNULL(cb.budget,0) AS budget
FROM categories c
...
if(!$categoryBudgetStatus){
    die("Category Budget Query Failed: " . $conn->error);
}
...
<a href="category_budget.php">Category Budgets</a>
...
<div class="card-container">
    ...
    <div class="card">
        <h3>Total Income</h3>
        <h1><?php echo number_format($incomeTotal); ?></h1>
    </div>

    <div class="card">
        <h3>Total Expense</h3>
        <h1><?php echo number_format($expenseTotal); ?></h1>
    </div>

    <div class="card">
        <h3>Net Savings</h3>
        <h1><?php echo number_format($netSavings); ?></h1>
    </div>
    ...
    <div class="card">
        <h3>Remaining</h3>
        <h1><?php echo number_format($remaining); ?></h1>
    </div>
...
<div class="card">
    ...
    <?php while($row = $categoryBudgetStatus->fetch_assoc()): ?>

        <p>
        <?php 
            $spent = $row['spent'];
            $budgetVal = $row['budget'];

            echo $row['category_name']." : ₹".
                 number_format($spent)." / ₹".
                 number_format($budgetVal);
